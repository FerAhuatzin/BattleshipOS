...
void retrieveBoats () {
	...
	for (int i=0; i<17; i++) {
		...
		printf("Coordeante of boat %d board A: %d,%d\n", i+1, XcoordenatesBoatsBoardA[i], YcoordenatesBoatsBoardA[i]);
		printf("Coordeante of boat %d board B: %d,%d\n", i+1, XcoordenatesBoatsBoardB[i], YcoordenatesBoatsBoardB[i]);
	} //end for
...
int checkShootCoordenate(int coordenateX,int coordenateY, int turn) {
	int notPreviousShot;
	if (coordenateY<=N && coordenateX<=N && coordenateY>=0 && coordenateX>=0) {
			notPreviousShot = checkNotPreviousShot(coordenateX,coordenateY,turn);
			if (notPreviousShot==1) {
				alarm(1);
				pause();
...
void play () {
	...
	while (win==0) {
		if (win==0) {
			printf ("The enemy has hit %d boats, keep it up!\n", hitsB );
			shoot(0);
		} //end
		if (win==0) {
			changeTurn();
